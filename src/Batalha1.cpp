...
int main(){
    ...
    int opcao,cont1=0,cont2=0;//Opcao � para escolha das op��es logo no inicio do algoritmo; O cont � usado para saber se o jogador ja atingiu o n�mero de preenchemento do barco
    int barco11,barco21,barco31,barco41,barco51,barco12,barco22,barco32,barco42,barco52;
     ...
     cin>>opcao;
            ...
            mostrarTab(tabela1);//Como tabela � igual no inicio ,ent�o � so preciso mostrar uma tabela que vale para os dois
            int l1,l2,c1,c2;
            char p;
            string b;
            for(int q=0;q<10;q++){//10 escolhas(5 para o primeiro jogador e o resto para o outro).
                    if((q%2)==0){//Determina o preenchimento altaternado dos jogadores.
                            ...
                            cout<<"\n";
                            cout<<"Preencha 5 barcos de diferentes tamanhos na tabela!\n1 - Barco de dimens�o 2!\n2 - Barco de dimens�o 3!\n3 - Barco de dimens�o 4!\n4 - Barco  de dimens�o 3!\n5 - Barco de dimens�o 5!"<<endl;
                            cout<<"Diga a linha e coluna que deseja preencher o barco "<<cont1+1<<endl;
                             ...
                             do{
                                ...
                                cin >> l1;
                                if(l1<0 || l1 >=10){
                                cout<<"Erro: A linha tem que ser entre 0 e 9, mas conforme a dimens�o do barco!!!."<<endl;
                            }
                            } while (l1 < -1 || l1 >= 10);
                            ...
                            do {
                                ...
                                cin >> c1;
                                if(c1<0 || c1 >=10){
                               cout<<"Erro: A coluna tem que ser entre 0 e 9, mas conforme a dimens�o do barco!!!."<<endl;
                            }
                            } while (c1 < -1 || c1 >= 10);


                            do{
                            cout<<"Deseja na posi��o horizontal(h) ou vertical(v):";
                            cin>>p;
            ...
            sleep(2);
            cout<<"       EST� A COME�AR O JOGO"<<endl;
            sleep(3);
            system("cls");
...
cout<<"\n"<<endl;
            ...
            int ContJogador=0;//Determina que jogador � que tem jogar
            bool jogo=true;
            bool Analise=true;
            ...
            while(jogo==true){
                if((ContJogador%2)==0){
                    ...
                    do {
                        cout << "Que linha � que desejas atingir o adversario: ";
                        cin >> TiroLinha1;

                        cout << "Que coluna � que desejas atingir o adversario: ";
                        cin >> TiroColuna1;

                        if (TiroLinha1 < -1 || TiroLinha1 >= 10 || TiroColuna1 < -1 || TiroColuna1 >= 10) {//Verifica se a linha e a coluna pertencem a matriz
                            cout << "As coordenadas est�o fora da matriz!!! Tenta novamente." << endl;
                            Analise = false;//Analise torna falso caso a linha ou coluna est� fora da matriz
                        } else if (tabela2[TiroLinha1][TiroColuna1] == " " || tabela2[TiroLinha1][TiroColuna1] == "X") {//Verifica se n�o houve tiro no espa�o escolhido
                            cout << "J� houve tiro neste espa�o. Escolha outro espa�o. Tenta novamente." << endl;
                            Analise = false;//Analise torna falso caso houver tiro no espa�o escolhido
                            } else {
                                Analise = true;
                            }
                        } while (!Analise);//Se Analise for falso vai mandar repetir tudo outra vez ou seja o espa�o que deseja atirar
                ...
                if(Analise==true){
                    ...
                    system("cls");
                    //Verifica�ao do tabuleiro do jogador 2
                ...
                if(verTabuleiro2==83){//10*10=100;100-17=87; 17 � n�mero de todas as partes do barco
                        jogo=false;
                    cout<<"+*~^+*~^+*~^+*~^+*~^+*~^+*~^+*~^+*~^ PARABENSSSSS +*~^+*~^+*~^+*~^+*~^+*~^+*~^+*~^+*~^"<<endl;
                            cout<<"                                 A vit�ria � do JOGADOR 2                          "<<endl;
                }
                ...
                TabelaAtualizaComTiros(tabela2);
                    ...
                    do {
                        ...
                        if (TiroLinha2 < -1 || TiroLinha2 >= 10 || TiroColuna2 < -1 || TiroColuna2 >= 10) {//Verifica se a linha e a coluna pertencem a matriz
                            cout << "As coordenadas est�o fora da matriz!!! Tenta novamente." << endl;//
                            Analise = false;//Analise torna falso caso a linha ou coluna est� fora da matriz
                        } else if (tabela1[TiroLinha2][TiroColuna2] == " " || tabela1[TiroLinha2][TiroColuna2] == "X") {//Verifica se n�o houve tiro no espa�o escolhido
                            cout << "J� houve tiro neste espa�o. Escolha outro espa�o. Tenta novamente." << endl;
                            Analise = false;//Analise torna falso caso houver tiro no espa�o escolhido
                            } else{
                                Analise = true;
                        ...
                        //Verifica��o da existencia do barco
                ...
                //Verifica�ao do tabuleiro do jogador 1
                for(int i=0;i<10;i++){
                    for(int j=0;j<10;j++){
            ...
            cout<<"\n\n\n\n\n\n\n\n";
            cout<<"                                                                   Siga a pagina para ficar a par das atualiza��es!!!"<<endl;
            sleep(5);
            exit(0);
            }
...
void preenchimento(string tab[tamanho][tamanho], int tamanho_barco, int linha, int coluna, char direcao, string tipo_do_barco) {
    ...
    // Verifica��es gerais de posi��o
    if (linha < 0 || linha >= tamanho || coluna < 0 || coluna >= tamanho) {
        posicaoValida = false;
    }

    if (posicaoValida) {
        // Verifica��o de posi��o e preenchimento na horizontal
        if ((direcao == 'h' || direcao == 'H') && coluna + tamanho_barco - 1 < tamanho) {//Verifica se o barco cabe na linha introduzido pelo utilizador e se caber entra nesse loop
            for (int i = 0; i < tamanho_barco; i++) {
                if (coluna + i >= tamanho || tab[linha][coluna + i] != "A") {//Se o esp�o selecionado tiver barco , entra nesse loop
                    posicaoValida = false;////Caso a condi��o seja verdade , declara  a posicaValida em falso
                    break;//Depois para o programa e salta para onde a posi�ao � invalida(no final dessa fun�ao)
                }
            }

            if (posicaoValida) {//No espa�o introduzido pelo utilizador, n�o ha barco/s
                for (int i = 0; i < tamanho_barco; i++) {
                    tab[linha][coluna + i] = tipo_do_barco;
                }
            }
        }
        // Verifica��o de posi��o e preenchimento na vertical
        ...
        // Verifica��o de posi��o e preenchimento na horizontal (inversa)
        ...
        // Verifica��o de posi��o e preenchimento na vertical (inversa)
    ...
    if (!posicaoValida){
        cout << "Posi��o ou dire��o inv�lida! Tenta outra vez!!!." << endl;
        cout << "Posi��o inv�lida! Tenta outra vez!!!." << endl;
        cout << "Informe a linha do seu preenchimento (entre 0 e 9): ";
        cin >> linha;
        ...
        preenchimento(tab, tamanho_barco, linha, coluna, direcao,tipo_do_barco);//Volta perguntar isso sempre que for a posi��o invalida
...
bool BarcoAfundado(string tab[tamanho][tamanho], string tipoBarco) {
    // Verifica se todas as posi��es do barco est�o marcadas como atingidas (X)
    for (int i = 0; i < tamanho; ++i) {
        for (int j = 0; j < tamanho; ++j) {
            if (tab[i][j] == tipoBarco) {
                return false; // Ainda h� uma parte do barco n�o atingida
            }
        }
    }

    return true; // Todas as partes do barco foram atingidas, o barco est� afundado
}
